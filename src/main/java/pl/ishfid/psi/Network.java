...
public class Network {
    ...
    public Network(int inputs, int hiddenLayers, int neuronInEachHiddenLayer, int outputs, double learningRate) throws IOException {
        this.hiddenLayers = new ArrayList<Layer>();
        this.learningRate = learningRate;
        this.inputLayer = new Layer();
        this.outputLayer = new Layer();
        this.dataManager = new DataManager();
        this.dataManager.importData("digits.txt");
        ...
        for (int i = 0; i < hiddenLayers; ++i)
        {
            ...
            else
            {
                ...
                this.hiddenLayers.add(new Layer()); // inserting new layer
                this.hiddenLayers.get(this.hiddenLayers.size() - 1).fillLayer
    ...
    public void feedForward(){
        ...
        ArrayList<McCullohPitts> lastHiddenLayerNeurons = this.hiddenLayers.get
    ...
    public void updateSignalErrors(){
        ...
        for (int layer = lastHiddenLayer; layer >= 0; --layer){
            ArrayList<McCullohPitts> actualHiddenLayerNeurons = this.hiddenLayers.get(layer).getNeurons();
            for (int neuron = 0; neuron < actualHiddenLayerNeurons.size(); ++neuron){
                double signalError = 0;

                if (layer == lastHiddenLayer){
                    ArrayList<McCullohPitts> outputLayerNeurons = this.outputLayer.getNeurons();
                    ...
                    ArrayList<McCullohPitts> nextHiddenLayerNeurons = this.hiddenLayers.get(layer + 1).getNeurons();
    ...
    public void updateWeights(){
        ...
        for (int i = 0; i < this.hiddenLayers.size(); ++i){
            ...
            for (int j = 0; j < layerSize; ++j){
                ...
                ArrayList<McCullohPitts> neurons = this.hiddenLayers.get(i).getNeurons();
        ...
        for (int i = 0; i < this.outputLayer.getSize(); ++i){
            ...
            ArrayList<McCullohPitts> neurons = this.outputLayer.getNeurons();
